...
// in when done; the list is grouped by type. Each task is
// { id, title, done, type, created_at } on profile.tasks.
export default function TareasSection({ rootOnBack }) {
  ...
  const add = () => {
    ...
    if (!title) return;
    // New tasks land at the bottom (of their type group).
    setTasks((l) => [...l, { id: newId(), title, done: false, type: draftType, created_at: new Date().toISOString() }]);
    setDraft('');
  };
  const toggle = (id) =>
  ...
  // Group by type (in TYPE_ORDER); keep insertion order within a group so
  // freshly added tasks show up at the bottom of their group.
  const groups = TYPE_ORDER
    .map((tp) => ({ type: tp, items: tasks.filter((x) => typeOf(x) === tp) }))
        ...
        {showTypePicker && (
          <div className="tareas-type-picker">
            {TYPE_ORDER.map((tp) => (
              ...
              >
                <TaskShape type={tp} done={draftType === tp} size={22} />
              </button>
            ))}
          </div>
        )}
        ...
        {tasks.length === 0 ? (
          ...
          <>
            <div className="tareas-groups">
              {groups.map((g) => (
                <ul key={g.type} className="tareas-list">
                  {g.items.map((task) => {
